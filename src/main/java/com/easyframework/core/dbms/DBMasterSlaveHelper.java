...
import com.easyframework.core.db.ConnectionFactory;
import com.easyframework.core.db.StringUtil;

public class DBMasterSlaveHelper {
	private static final Logger log = Logger.getLogger(DBMasterSlaveHelper.class);
		...
		try {
			if (connection == null || connection.isClosed()) {
				...
				if(LoadPropertiesUtils.Is_MS_OPEN&&LoadPropertiesUtils.db_Master_W){
					...
					if(null==connection){connection =ConnUtils.initConnection();}
				}
			}
		...
		try {
			try {
				if (connectionMaster == null || connectionMaster.isClosed()) {
					//connection = ConnectionFactory.getInstance().getConnection();
					if(LoadPropertiesUtils.Is_MS_OPEN&&LoadPropertiesUtils.db_Master_W){
						connectionMaster =MasterConnectionFactory.getInstance().getConnection();
					}
				}
		...
		try {
			...
			for (int index = 0; index < obj.length; index++) {
				...
				if (name.length() > 0 && value.length() > 0) {
					...
					try {
						dialect = MasterConnectionFactory.DIALECT;
					} catch (Exception e2) {
						// TODO Auto-generated catch block
						e2.printStackTrace();
					}
