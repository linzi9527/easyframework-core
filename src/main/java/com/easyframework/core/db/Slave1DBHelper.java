...
public class Slave1DBHelper {
	private static final Logger log = Logger.getLogger(Slave1DBHelper.class);
		...
		try {
			...
			for (int index = 0; index < obj.length; index++) {
				...
				if (name.length() > 0 && value.length() > 0) {
					...
					// 判断方言Oracle，必须加上自增序列
					if (Slave1ConnectionFactory.getDialect().toLowerCase().equals("mysqldialect")) {
						sb_index.append("(").append(name).append(")").append("  VALUES").append("(").append(value).append(")");
					} else if (Slave1ConnectionFactory.getDialect().toLowerCase().equals("oracledialect")) {
